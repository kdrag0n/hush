...
#[command(
    name = "hush-server",
    version,
    about = "Server for hush, an SSH-like remote shell over KCP",
    long_about = "hush-server listens for KCP connections, authenticates clients with Ed25519 SSH keys, and runs remote shell sessions.\n\nServer files live under the data directory's server/ subdirectory. The default config path is $DATA_DIR/server/config.toml, and the default raw Noise host key is $DATA_DIR/server/host_key.noise. When running as root, the default data directory is /etc/hush. Command-line flags override config file values."
)]
pub(crate) struct Args {
    ...
    /// Maximum concurrent KCP connections.
    #[arg(long, value_name = "N", help_heading = "Limits")]
    pub(crate) max_connections: Option<usize>,

    /// Maximum shell sessions accepted per KCP connection.
    #[arg(long, value_name = "N", help_heading = "Limits")]
