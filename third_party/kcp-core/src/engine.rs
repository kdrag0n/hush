...
impl KcpEngine {
    ...
    /// Number of KCP data segments queued locally or sent but not yet ACKed.
    pub fn pending_send_segments(&self) -> usize {
        self.snd_queue.len() + self.snd_buf.len()
    }

    /// Configured local send window in KCP segments.
    pub fn send_window_segments(&self) -> usize {
        self.wnd.snd as usize
    }

    /// Number of KCP segments needed to carry a write of `len` bytes.
    pub fn segments_for_data_len(&self, len: usize) -> usize {
        if len == 0 {
            0
        } else {
            len.div_ceil(self.mss() as usize)
        }
    }
